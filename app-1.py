...
def list_photos(slug):
    ...
    for r in res.get("resources", []):
        ...
        photos.append({
            ...
            "download_url": cloudinary.CloudinaryImage(public_id).build_url(flags="attachment"),
...
def download_photos(slug):
    ...
    signs the request, so downloading a big batch doesn't load this server
    any more than uploading one does now."""
    slug = slugify(slug)

    if request.form.get("all"):
        public_ids = [p["public_id"] for p in list_photos(slug)]
    else:
        prefix = f"{ALBUMS_ROOT}/{slug}/"
        # trust only public_ids that actually belong to this album, in case
        # the form was tampered with
        public_ids = [pid for pid in request.form.getlist("public_ids") if pid.startswith(prefix)]

    if not public_ids:
        flash("Pick at least one photo to download.", "error")
        return redirect(url_for("album", slug=slug))

    if len(public_ids) == 1:
        return redirect(cloudinary.CloudinaryImage(public_ids[0]).build_url(flags="attachment"))
    ...
    try:
        ...
        logger.exception("Could not build a download link for album %r", slug)
        flash("Couldn't put that download together — try again.", "error")
        return redirect(url_for("album", slug=slug))
