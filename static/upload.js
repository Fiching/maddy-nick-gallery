    ...
    return new Promise(function (resolve, reject) {
      ...
      xhr.addEventListener("load", function () {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          // Cloudinary sends back a JSON body like {"error":{"message":"..."}}
          // explaining exactly why -- surface that instead of a generic
          // message, since this request never touches our own server/logs.
          var reason = "rejected (" + xhr.status + ")";
          try {
            var body = JSON.parse(xhr.responseText);
            if (body && body.error && body.error.message) reason = body.error.message;
          } catch (e) {
  ...
  function runBatch(files, skippedCount) {
    ...
    function worker() {
      ...
      return requestSignature()
        ...
        .catch(function (err) {
          loaded[i] = file.size;
          failed.push({ file: file, reason: (err && err.message) || "unknown error" });
        })
        .then(function () {
          finishedCount++;
          updateProgress();
    ...
    function finish() {
      ...
      // Show *why*, not just how many -- these fail on the direct line from
      // the browser to Cloudinary, which never touches our own server, so
      // this message is the only place the real reason is visible at all.
      var reasonCounts = {};
      failed.forEach(function (f) {
        reasonCounts[f.reason] = (reasonCounts[f.reason] || 0) + 1;
      });
      var reasonSummary = Object.keys(reasonCounts).map(function (r) {
        return reasonCounts[r] > 1 ? r + " (×" + reasonCounts[r] + ")" : r;
      }).join("; ");

      progressLabel.textContent = succeededCount + " of " + files.length + " uploaded, " +
        failed.length + " failed — " + reasonSummary +
        ". The ones that made it are already saved — retry just the rest, or open the album now.";
      retryQueue = failed.map(function (f) { return f.file; });
